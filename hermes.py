...
if __name__ == "__main__":
    ...
    with st.sidebar:
        selected = option_menu("HermesViz",
    ...
    st.title("Welcome to my HermesViz 🎶")

    if selected == "Presentation":

        from PIL import Image

        hermesImage = Image.open('hermes1.jpg')
        hermesImage2 = Image.open('hermes2.jpg')


        st.image(hermesImage, width=600)
        st.title("Presentation 🎤")
        st.markdown("Depuis 1837 avec Thierry Hermès, la maison de luxe Hermès n'a cessé d'inspirer les nouvelles tendance tout en gardant son authenticité. "
                    "Cette réussite s'est effevtuée avec la compréhension et l'anticipation des attentes de ses clients. De nos jours, cette prévision peut s'améliorer "
                    "grâce à l'analyse des données des plateformes d'interaction client comme Instagram.")
        st.image(hermesImage2,width=400)
        ...
        st.markdown('La data visualisation que je vais vous présenter se base sur votre compte instagram, et montre des statistiques sur le nombre de likes ou de comments '
                    ', le comportement de ces derniers à travers les jours et heures, et bien plus encore ... Elle se déroule du mois d août à Octobre 2022.  Bonne visualisation ❤️.')
    ...
    if selected == "Transforming the Data":
        ...
        st.image(discover)
        st.title('Hermes Data')
        data_observation('hermes')
        ...
        st.subheader('hermes data after transform')
